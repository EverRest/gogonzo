...
class TIVWP_Updater {
	...
	public function filter__pre_set_site_transient_update_plugins( $transient ) {
		...
		if (
			...
			// Do we have data?
			or ! $this->_is_license_pair_filled_in()
		) {
			return $transient;
		}
	...
	public function filter__plugins_api( $result, $action, $args ) {
		...
		if ( empty( $action ) or 'plugin_information' !== $action ) {
			return $result;
		}
		...
		if ( isset( $response->sections ) ) {
			...
			if ( ! count( $response->sections ) ) {
				$response->sections['other_notes'] =
					'<p>' .
					esc_html__( 'Please visit the plugin page for more information',
						'tivwp-updater' ) . ':' .
					'</p>' .
					'<p><a href="' . $this->url_product . '">' . $this->url_product . '</a></p>';
			}
	...
	protected function _get_upgrade_api_response( array $request_parameters ) {
		...
		if (
			is_wp_error( $result )
			or ( 200 !== (int) wp_remote_retrieve_response_code( $result ) )
		) {
